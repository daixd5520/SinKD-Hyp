...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--model_name_or_path",
        type=str,
        default="/youtu-reid/data/t-zero/T0pp",
        required=True,
        help=(
...
def main():
    ...
    model_namet = ""
    model_namet_new = os.path.join("",args.dataset_config_name)
    tmodel = AutoModelForSeq2SeqLM.from_pretrained(model_namet_new)
    ttokenizer = AutoTokenizer.from_pretrained(model_namet)
    ...
    for epoch in range(1, 11):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                #     #tqdm.write(f"epoch = {epoch}, step = {global_steps}, loss = {loss},klloss={loss1}")
                # if args.wandb_proj and accelerator.is_main_process:
        ...
        # Evaluate every epoch
                ...
                for batch1 in eval_dataloader:
                    ...
                    with torch.no_grad():
                        ...
                        result=model.generate(model_inputs['input_ids'])
                    masked_log_probs = batch1["labels_attention_mask"].unsqueeze(-1) * torch.log_softmax(logits, dim=-1)
                    seq_token_log_probs = torch.gather(masked_log_probs, -1, batch1["labels"].unsqueeze(-1))
                    seq_log_prob = seq_token_log_probs.squeeze(dim=-1).sum(dim=-1)
                    seq_log_prob = seq_log_prob.view(batch1["targets"].size(0), -1) #TODO(Victor): this reshapes works based on the assumption that all examples have the same number of choices. the pre-processing doesn't make this assumption.
